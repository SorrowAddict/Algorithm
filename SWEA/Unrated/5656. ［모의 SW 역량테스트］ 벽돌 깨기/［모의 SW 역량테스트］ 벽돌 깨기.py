...
def shoot(level, remains, now_arr):
    ...
    if level == N or remains == 0:
        min_blocks = min(min_blocks, remains)   # 최솟값 갱신
        return
    ...
    for col in range(W):
        ...
        # 2. 연쇄적으로 벽돌이 깨짐
        # 행, 열, 숫자(파워)
        li = [(row, col, copy_arr[row][col])]  # 앞으로 꺠져야 할 벽돌들을 저장
...
for tc in range(1, T+1):
    ...
    # 2. 현재 벽돌이 다 꺠지면(남은 벽돌이 없다) 더 이상 진행할 필요가 없다 !
    #   - 현재 벽돌 수를 같이 저장하면 좋을 것 같다.
    # 3. N 번 구슬을 쏘자.
    #   - 시작점: 0번 / 하나도 안깨진 벽돌 수
